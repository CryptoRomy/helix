...
from test_framework.test_framework import HelixTestFramework
...
class ReorgStakeTest(HelixTestFramework):
    ...
    def check_money_supply(self, expected_hlix, expected_zhlix):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected HLIX and zHLIX supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_hlix))
            for denom in node['zHLIXsupply']:
                assert_equal(node['zHLIXsupply'][denom], DecimalAmt(expected_zhlix[denom]))
    ...
    def run_test(self):
        ...
        # Check HLIX and zHLIX supply at the beginning
        # ------------------------------------------
        # zHLIX supply: 2 coins for each denomination
        expected_zhlix_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # HLIX supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zhlix_supply)
        ...
        # Verify that HLIX and zHLIX supplies were properly updated after the spends and reorgs
        self.log.info("Check HLIX and zHLIX supply...")
        ...
        expected_zhlix_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zhlix_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zhlix_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zhlix_supply)
