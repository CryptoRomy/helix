...
class ZerocoinSpendTest(HelixTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zhlix_bal, hlix_bal):
            zhlix_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zhlix_bal)
            hlix_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], hlix_bal)
            return zhlix_bal, hlix_bal
        ...
        zhlix_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zhlix_balance, 6666)
        ...
        zhlix_balance, balance = check_balances(denom_2, zhlix_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zhlix_balance, balance = check_balances(denom_3, zhlix_balance, balance)
        ...
        _, _ = check_balances(denom_1, zhlix_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
