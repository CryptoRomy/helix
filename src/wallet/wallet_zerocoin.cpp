...
#include "zhlixchain.h"
#include "zhlix/deterministicmint.h"
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZHLIX_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZHLIX_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZHLIX_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZHLIXModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZHLIX_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zHLIX public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zHLIX used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zhlixTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zHLIX in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zhlixTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zhlixTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzHLIXWallet* zwallet)
{
    zwalletMain = zwallet;
    zhlixTracker = std::unique_ptr<CzHLIXTracker>(new CzHLIXTracker(this));
}

CzHLIXWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zhlixTracker->ListMints(false, false, true);
