...
bool CLegacyZHlixStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZHlixStake from non zc-spend");
    ...
    *this = CLegacyZHlixStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zhlix stake origin", __func__);

    // All good
    return true;
}

CLegacyZHlixStake::CLegacyZHlixStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZHlixStake::GetIndexFrom()
{
...
CAmount CLegacyZHlixStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZHlixStake::GetUniqueness() const
{
...
bool CLegacyZHlixStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (nHeight < consensus.height_start_ZC_SerialsV2 || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zHLIX stake block: height %d outside range", __func__, nHeight);
