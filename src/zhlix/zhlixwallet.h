...
#ifndef HELIX_ZHLIXWALLET_H
#define HELIX_ZHLIXWALLET_H
...
public:
    CzHLIXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZHLIX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZHLIX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //HELIX_ZHLIXWALLET_H
