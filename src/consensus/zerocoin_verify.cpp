...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZHLIXModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZHLIXModule::validateInput(txin, prevOut, tx, ret)){
    ...
    if (nHeight >= consensus.height_start_ZC_SerialsV2) {
        try {
            if (!spend->HasValidSignature())
                return error("%s: V2 zHLIX spend does not have a valid signature\n", __func__);
        ...
        if (spend->getSpendType() != expectedType) {
            return error("%s: trying to spend zHLIX without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
...
bool RecalculateHLIXSupply(int nHeightStart, bool fSkipZhlix)
{
    ...
    uiInterface.ShowProgress(_("Recalculating HLIX supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating HLIX supply..."), percent);
        }
        ...
        // Rewrite zhlix supply too
        if (!fSkipZhlix && pindex->nHeight >= consensus.height_start_ZC) {
