...
bool CHlixStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CHlixStake from zerocoin spend");
...
bool CHlixStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CHlixStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CHlixStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CHlixStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CHlixStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CHlixStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CHlixStake::GetUniqueness() const
{
    //The unique identifier for a HLIX stake is the outpoint
...
CBlockIndex* CHlixStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CHlixStake::ContextCheck(int nHeight, uint32_t nTime)
{
