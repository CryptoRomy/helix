// Copyright (c) 2018-2020 The HELIX developers
...
std::string ReindexZerocoinDB()
{
    ...
    while (pindex) {
        ...
        // update supply
        UpdateZHLIXSupplyConnect(block, pindex, true);
                    ...
                    if (tx.HasZerocoinSpendInputs()) {
                        for (auto& in : tx.vin) {
                            ...
                            if (isPublicSpend) {
                                ...
                                CValidationState state;
                                if (!ZHLIXModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
{
    CDataStream serializedCoinSpend = ZHLIXModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZHLIXSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    //Add mints to ZHLIX supply (mints are forever disabled after last checkpoint)
    ...
    // A one-time event where only the ZHLIX supply was off (due to serial duplication off-chain on main net)
...
bool UpdateZHLIXSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
    ...
    // Re-add spends to ZHLIX supply
    std::list<libzerocoin::CoinDenomination> listDenomsSpent = ZerocoinSpendListFromBlock(block, true);
    for (const libzerocoin::CoinDenomination& denom : listDenomsSpent) {
        mapZerocoinSupply.at(denom)++;
    }

    // Remove mints from ZHLIX supply (mints are forever disabled after last checkpoint)
    if (pindex->nHeight < consensus.height_last_ZC_AccumCheckpoint) {
        std::list<CZerocoinMint> listMints;
        std::set<uint256> setAddedToWallet;
        BlockToZerocoinMintList(block, listMints, true);
